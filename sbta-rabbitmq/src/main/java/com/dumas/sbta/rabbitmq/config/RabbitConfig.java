package com.dumas.sbta.rabbitmq.config;
...
public class RabbitConfig {
    ...
    public RabbitTemplate rabbitTemplate(CachingConnectionFactory connectionFactory){
        ...
        rabbitTemplate.setConfirmCallback(((correlationData, ack, cause) -> log.info("消息发送成功：correlationData({}), cause({})", correlationData, ack, cause)));
        rabbitTemplate.setReturnCallback(((message, replyCode, replyText, exchange, routing) -> log.info("消息丢失：exchange({}), route({}), replyCode({}), replyText({}), message：{}", exchange, routing, replyCode, replyText, message)));
